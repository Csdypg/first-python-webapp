...
class _DbCtx(threading.local):
	...
	def init(self):
		...
		logging.info('open lazy connection ok.')
		self.transactions = 0
...
class _TransactionCtx(object):
	...
	def commit(self):
		global _db_ctx
		logging.info('commint tarasaction...')
		try:
			_db_ctx.connection.commit()
			logging.info('commit ok.')
		except:
			logging.info('commit failed, try rollback...')
...
def _select(sql, first, *args):
	'''execute select SQL and renturn unique result or list results.'''
	global _db_ctx
	cursor = None
	sql = sql.replace('?','%s')
